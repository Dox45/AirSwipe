...
export default function EventCard({ eventId }: { eventId: Id<"events"> }) {
  ...
  return (
    <div
      onClick={() => router.push(`/event/${eventId}`)}
      className={`bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 border border-gray-100 cursor-pointer overflow-hidden relative ${
        isPastEvent ? "opacity-75 hover:opacity-100" : ""
      }`}
    >
      {/* Event Image */}
      ...
      <div className="p-5 space-y-3">
        ...
        <div className="flex justify-between items-start w-full">
          ...
          <div className="flex flex-col items-end gap-1">
            <ShareEventButton eventName={event.name} eventId={eventId} />
            <span className="px-3 py-1.5 text-sm font-semibold rounded-full bg-green-100 text-green-700 whitespace-nowrap">
              N{event.price.toFixed(2)}
            </span>
          </div>
        </div>
          ...
          <div className="flex items-center flex-wrap gap-1">
            ...
            {availability.activeOffers > 0 && (
              <span className="text-amber-600 text-xs ml-2">
                ({availability.activeOffers} trying to buy)
              </span>
            )}
          </div>
        </div>
        ...
        {userTicket && (
          <div className="p-3 bg-green-50 rounded-lg border border-green-200 flex justify-between items-center">
            ...
            <button
              onClick={() => router.push(`/tickets/${userTicket._id}`)}
              className="text-xs bg-green-600 hover:bg-green-700 text-white px-3 py-1.5 rounded-full font-medium shadow-sm transition-colors duration-200"
            >
              View Ticket
            </button>
          </div>
        ...
        {/* Purchase Button */}
        {!isPastEvent && !userTicket && <PurchaseTicket eventId={eventId} />}
      </div>
    </div>
  );
}
